...
point_cloud_range = [-84.48, -84.48, -3.5, 84.48, 84.48, 3.5]

# model config
voxel_size = [0.32, 0.32, 7.0]
model = dict(
    type='CPDet',
    ...
    pts_middle_encoder=dict(
        type='PointPillarsScatter', in_channels=64, output_shape=(528, 528)),
    pts_backbone=dict(
        type='SECOND',
        in_channels=64,
...
data_root = 'data/qa_for_mmdet3d/kitti_format/'
file_client_args = dict(backend='disk')
input_modality = dict(use_lidar=True, use_camera=False)
db_sampler = dict(
    data_root=data_root,
    info_path=data_root + 'qa_dbinfos_train.pkl',
    rate=1.0,
    prepare=dict(
...
data = dict(
    ...
    train=dict(
        ...
        dataset=dict(
            type=dataset_type,
            data_root=data_root,
            ann_file=data_root + 'qa_infos_train.pkl',
            split='training',
    ...
    val=dict(
        type=dataset_type,
        data_root=data_root,
        ann_file=data_root + 'qa_infos_val.pkl',
        split='training',
