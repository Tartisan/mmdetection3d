...
from tools.onnx_tools.debug import Debugger
...
class PillarFeatureNet(nn.Module):
    ...
    def forward(self, voxel_features):
        ...
        tmp = voxel_features.squeeze(1)
        np.save('11_pfe_net_feature.npy', tmp.cpu().detach().numpy())
        return tmp
...
class Backbone(nn.Module):
    ...
    def forward(self, x): 
        x = self.pts_backbone(x)
        x0 = debugger.debug(x[0], 'backbone_0')
        x1 = debugger.debug(x[1], 'backbone_1')
        x2 = debugger.debug(x[2], 'backbone_2')
        x = (x0, x1, x2)
        x = self.pts_neck(x)
        x0 = debugger.debug(x[0], 'neck')
        x = [x0]
        x = self.pts_bbox_head(x)
        
        # bbox_preds, scores, dir_scores = [], [], []
        out = {}
        for i, task_res in enumerate(x):
            reg = debugger.debug(task_res[0]['reg'], 'reg_' + str(i))
            height = debugger.debug(task_res[0]['height'], 'height_' + str(i))
            dim = debugger.debug(task_res[0]['dim'], 'dim_' + str(i))
            heatmap = debugger.debug(task_res[0]['heatmap'], 'heatmap_' + str(i))
            rot = debugger.debug(task_res[0]['rot'], 'rot_' + str(i))

            out['reg_' + str(i)] = reg
            out['height_' + str(i)] = height
            out['dim_' + str(i)] = dim
            out['heatmap_' + str(i)] = heatmap
            out['rot_' + str(i)] = rot
...
def main():
    ...
    RELATIVE_PATH = 'tools/onnx_tools/centerpoint/files/'
    ...
    input_array = np.loadtxt(RELATIVE_PATH + '00_voxel_feature.txt', dtype='float32')
    ...
    # np.save('voxel_features.npy', voxel_features.cpu().detach().numpy())
    batch_size = coors[-1, 0] + 1
    # 64 * 800 * 800
    scattered_features = model.pts_middle_encoder(
        voxel_features, coors, batch_size)
    # np.save('scattered_features.npy', 
    #         scattered_features.squeeze(0).cpu().detach().numpy())

    # scattered_feature_trt = np.loadtxt(
    #     RELATIVE_PATH + '04_canvas_feature.txt', dtype='float32')
    # scattered_features = torch.from_numpy(scattered_feature_trt.reshape(1, 64, 800, 800)).to(device)
    backbone_model = build_backbone_model(cfg, args.checkpoint, device)
    torch.onnx.export(backbone_model,
                      scattered_features,
                      f='./tools/onnx_tools/centerpoint/before_backbone.onnx',
                      opset_version=12,
                      verbose=True,
                      input_names=['scattered_features'],
                      do_constant_folding=True)

    debugger.extract_debug_model(
        './tools/onnx_tools/centerpoint/before_backbone.onnx', 
        './tools/onnx_tools/centerpoint/after_backbone.onnx')
    debugger.run_debug_model(
        {'scattered_features':scattered_features.cpu().detach().numpy()}, 
        './tools/onnx_tools/centerpoint/after_backbone.onnx') 
