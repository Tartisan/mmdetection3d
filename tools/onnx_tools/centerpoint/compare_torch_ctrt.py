...
def main():
    ...
    pillar_num = voxels.shape[0]

    # PillarFeatureNet
    pts_voxel_encoder = build_pfn_model(cfg, args.checkpoint, device=device)
    # max_voxels * 20 * 10
    input_array = np.loadtxt(LOG_PATH + '00_voxel_feature.txt', dtype='float32')
    input_array = input_array.reshape(cfg.model['pts_voxel_layer']['max_voxels'][1],
                                      cfg.model['pts_voxel_layer']['max_num_points'],
                                      pts_voxel_encoder.in_channels)
    dummy_input = torch.from_numpy(input_array).to(device)
    voxel_encoder_output = pts_voxel_encoder.forward(dummy_input)[:pillar_num]
    pfe_net_feature_pytorch = voxel_encoder_output.cpu().detach().numpy()
    
    pfe_net_feature_trt = np.loadtxt(
        LOG_PATH + '03_pillar_feature.txt', dtype='float32')[:pillar_num]
    diff = np.abs(pfe_net_feature_pytorch - pfe_net_feature_trt)
    print(f"\npfe_net_feature shape:{diff.shape} with L1 diff {np.sum(diff)}, min diff:{np.min(diff)}, max diff:{np.max(diff)}")


    pillar_coors = np.loadtxt(
        LOG_PATH + '02_voxel_coors.txt', dtype='int32')[:pillar_num]
    coors = torch.from_numpy(pillar_coors).to(device)
    batch_size = coors[-1, 0] + 1
    # 64 * 800 * 800
    middle_encoder_output = model.pts_middle_encoder(
        voxel_encoder_output, coors, batch_size)
    scattered_feature_pytorch = middle_encoder_output.cpu().detach().numpy().reshape(
        800 * 800, 64
    )
    scattered_feature_trt = np.loadtxt(
        LOG_PATH + '04_canvas_feature.txt', dtype='float32')
    diff = np.abs(scattered_feature_pytorch - scattered_feature_trt)
    print(f"\nscattered_feature shape:{diff.shape} with L1 diff {np.sum(diff)}, min diff:{np.min(diff)}, max diff:{np.max(diff)}")
    
    # Backbone
    backbone_model = build_backbone_model(cfg, args.checkpoint, device)
    backbone_output = backbone_model.forward(middle_encoder_output)

    scores_pytorch = backbone_output[0].cpu().detach().numpy().reshape(200 * 200, 4)
    bbox_preds_pytorch = backbone_output[1].cpu().detach().numpy().reshape(200 * 200, 18)
    dir_scores_pytorch = backbone_output[2].cpu().detach().numpy().reshape(200 * 200, 6)
    bbox_preds_trt = np.loadtxt(LOG_PATH + '05_bbox_preds.txt', dtype='float32')
    scores_trt = np.loadtxt(LOG_PATH + '06_cls_scores.txt', dtype='float32')
    dir_scores_trt = np.loadtxt(LOG_PATH + '07_dir_scores.txt', dtype='float32')

    diff = np.abs(bbox_preds_pytorch - bbox_preds_trt)
    print(f"\nbbox_preds shape:{diff.shape} with L1 diff {np.sum(diff)}, min diff:{np.min(diff)}, max diff:{np.max(diff)}")
    diff = np.abs(scores_pytorch - scores_trt)
    print(f"\nscores shape:{diff.shape} with L1 diff {np.sum(diff)}, min diff:{np.min(diff)}, max diff:{np.max(diff)}")
    diff = np.abs(dir_scores_pytorch - dir_scores_trt)
