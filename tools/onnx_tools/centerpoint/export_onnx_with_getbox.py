...
class Backbone(nn.Module):
    def __init__(self, cfg):
        ...
        pts_bbox_head.update(test_cfg=self.test_cfg.pts if self.test_cfg else None)
        self.pts_bbox_head = HEADS.build(pts_bbox_head)
        self.bbox_coder = build_bbox_coder(cfg['pts_bbox_head']['bbox_coder'])
        self.box_code_size = self.bbox_coder.code_size
        self.num_classes = [1, 1, 2]
        self.task_heads = [6, 6, 6]
...
def main():
    ...
    coors_trt = np.loadtxt(LOG_PATH + '02_voxel_coors.txt', dtype='int32')

    voxel_size = voxel_feature_trt.shape[0]
    voxel_feature_trt = voxel_feature_trt.reshape(
        voxel_size,
    ...
    pillar_feature_trt = np.loadtxt(
        LOG_PATH + '03_pillar_feature.txt', dtype='float32')
    ...
    pillar_feature = torch.from_numpy(pillar_feature_trt[:voxels.shape[0]]).to(device)
    coors = torch.from_numpy(coors_trt[:voxels.shape[0]]).to(device)
