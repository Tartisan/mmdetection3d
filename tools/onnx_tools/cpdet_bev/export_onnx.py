...
class Backbone(nn.Module):
    def __init__(self, cfg):
        ...
        input_feature_dim = cfg['pts_middle_encoder']['in_channels'] + \
                            cfg['pts_middle_encoder']['expand_bev_channels']
        out_channels = cfg['pts_middle_encoder']['out_channels']
...
def build_pfn_model(cfg, checkpoint=None, device='cuda:0'):
    ...
    for key in checkpoint['state_dict'].keys():
        if 'pfn' in key:
            dicts[key.split('pts_voxel_encoder.')[1]] = checkpoint['state_dict'][key]
...
def build_backbone_model(cfg, checkpoint=None, device='cuda:0'):
    ...
    dicts = {}
    for key in checkpoint["state_dict"].keys():
        if "middle_encoder" in key:
            dicts[key.replace('pts_middle_encoder.', '')] = checkpoint["state_dict"][key]
        if "backbone" in key:
...
def main():
    ...
    dummy_input = torch.ones(600000, 1, pts_voxel_encoder.in_channels, 1).to(device)
    torch.onnx.export(pts_voxel_encoder,
                      dummy_input,
                      f='./tools/onnx_tools/cpdet_bev/pfe.onnx',
                      opset_version=12,
                      verbose=True,
                      input_names=['voxels'],
    ...
    input_feature_dim = cfg['model']['pts_middle_encoder']['in_channels'] + \
                        cfg['model']['pts_middle_encoder']['expand_bev_channels']
    voxel_size = np.array(cfg['voxel_size'])
    point_cloud_range = np.array(cfg['point_cloud_range'])
    grid_size = (point_cloud_range[3:] - point_cloud_range[:3]) / voxel_size
    grid_size = np.round(grid_size).astype(np.int32)
    scattered_features = torch.ones(
        1, input_feature_dim, grid_size[1], grid_size[0]).to(device)

    backbone_model = build_backbone_model(cfg, args.checkpoint, device)
    torch.onnx.export(backbone_model,
                      scattered_features,
                      f='./tools/onnx_tools/cpdet_bev/backbone.onnx',
                      opset_version=12,
                      verbose=True,
