...
class RPNV2(BaseModule):
    def __init__(
        ...
        **kwargs
        ...
        self.deblock_5 = nn.Sequential(
            nn.ConvTranspose2d(num_out_filters, self._num_upsample_filters[1], 2, stride=2, bias=False),
    ...
    def _make_layer(self, inplanes, planes, num_blocks, stride=1):
        block = [
            build_conv_layer(self._conv_cfg, inplanes, planes, 3, stride=stride, padding=1),
            build_norm_layer(self._norm_cfg, planes)[1],
            nn.ReLU(inplace=True)
        ]
        for j in range(num_blocks):
            block.append(build_conv_layer(self._conv_cfg, planes, planes, 3, padding=1))
