...
class CPDet(MVXTwoStageDetector):
    ...
    @torch.no_grad()
    @force_fp32()
    def voxelize_with_bev(self, points):
        """Apply dynamic voxelization to points.
        ...
        Returns:
            tuple[torch.Tensor]: 
                Concatenated points, coordinates and bev_features.
        """
        coors = []
        bev_features = []
        for res in points:
            res_coors, res_bev_features = self.pts_voxel_layer(res)
            coors.append(res_coors)
            bev_features.append(res_bev_features)
        points = torch.cat(points, dim=0)
        coors_batch = []
        for i, coor in enumerate(coors):
            coor_pad = F.pad(coor, (1, 0), mode='constant', value=i)
            coors_batch.append(coor_pad)
        coors_batch = torch.cat(coors_batch, dim=0)
        bev_features_batch = torch.stack(bev_features)
        return points, coors_batch, bev_features_batch
    ...
    def extract_pts_feat(self, pts, img_feats, img_metas):
        ...
        if not self.with_pts_bbox:
            return None
        bev_features = None
        if self.use_custom_voxel_layer:
            voxels, coors, bev_features = self.voxelize_with_bev(pts)
        else:
        ...
        x = self.pts_middle_encoder(
            voxel_features, feature_coors, batch_size, bev_features)
        # x.cpu().numpy().tofile('work_dirs/quat/%06d.bin' % img_metas[0]['sample_idx'])
