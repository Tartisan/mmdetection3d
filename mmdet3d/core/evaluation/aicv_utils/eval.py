...
def get_thresholds(scores: np.ndarray, num_gt, num_sample_pts=41):
    """
    获取对于 recall 离散化必须要评估的scores
    当不再满足 r_recall - current_recall < current_recall - l_recall 时就退出
    在退出前，每有一个满足的 current_recall 就加1/40 --> 数值人为规定
    Args:
        scores: 所有有效gt匹配dt的分数
        num_gt: 全部有效box的数量
    Returns：
        thresholds: 41个分界点
    """
    ...
    for i, score in enumerate(scores):
        ...
        current_recall += 1 / (num_sample_pts - 1.0) # 每次增加 0.025
    return thresholds


def clean_data(gt_anno, dt_anno, current_class, difficulty):
    """
    根据 current_class 和 difficulty 对 gt 和 dt 中的目标进行分类标记，并统计有效目标的
    个数。分类标记原则是：
        1. 如果类别等于当前要评价的 current_class，且难度级别符合当前 difficulty，
           标记为 0；
        2. 如果类别相等，但是难度超出了当前 difficulty，或者由于 gt 的类别过于丰富，
           而要评价的 current_class 只是一个相近类别，标记为 1；
        3. 如果类别不等于 current_class，也不等于相近类别，标记为 -1；
    只有标记为 0 的目标会参与到后续 precision、recall 的计算中

    Args:
        gt_anno: 单帧点云标注dict
        dt_anno: 单帧点云预测dict
        current_class: 当前类别id
        difficulty: 难度id
    Returns:
        num_valid_gt: 有效gt的数量
        ignored_gt: gt标志列表 0：有效，1：忽略，-1：无效
        ignored_dt: dt标志列表 0：有效，1：忽略，-1：无效
    ...
    for i in range(num_gt):
        ...
        if (gt_name == current_cls_name):
            ...
            valid_class = -1
        ignore = False
        if ((gt_anno['occluded'][i] > MAX_OCCLUSION[difficulty]) or # 遮挡太严重
            (gt_anno['truncated'][i] > MAX_TRUNCATION[difficulty]) or # 截断太多
            (height <= MIN_HEIGHT[difficulty])): # 图片上对应的bbox像素高度太小
            ignore = True
...
def compute_statistics_jit(overlaps,
    ...
    # 遍历gt，这里以gt为主，针对gt找匹配的dt，跳过无效的gt和dt
    for i in range(gt_size):
        if ignored_gt[i] == -1:
            continue
        det_idx = -1 # 储存目前为止匹配dt的最佳idx
        valid_detection = NO_DETECTION # # 标记是否为有效dt
        max_overlap = 0 # 存储目前为止匹配dt的最佳overlap
